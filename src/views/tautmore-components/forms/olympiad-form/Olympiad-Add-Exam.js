...
import { formschema_olympiad_exam_tests } from "../../utility/schema/Fields_Schema";
import "../exams-form/ExamForm.scss";
import { getAllClass, getAllSubject, getTimeZone, getOlympiadExamTypes, addOlympiadExams, editOlympiadExams } from "../../services/apis/tautmore_exams_apis/tautmore_exams_apis";
...
class DataListSidebar extends Component {
  ...
  async getExamTypeList(grade,e){
    let subjectsIds = e.map((data)=>{
      return data.id
    })
    let subData = subjectsIds.toString();
    try{
      const params = {
        grade:grade.id,
        subject:subData
      }
      const res = await getOlympiadExamTypes(params);
  ...
  async addExamData(formData){
    try{
      const res = await addOlympiadExams(formData);
      if(res.status === 200){
  ...
  async editExamData(formData){
    try{
      const res = await editOlympiadExams(formData)
      if(res.status === 200){
...
//   }
  ...
  handleAddSubmit = (obj) => {
    ...
    console.log(obj);
    let subIdData = obj.subject.map((data)=>{
      return data.id
    })

    let subNameData = obj.subject.map((data)=>{
      return data.value
    })
    ...
    if(isEditAble){
      ...
      // console.log("editable", editFormData)
  ...
  render() {
    let { isEditAble, show, data, title, handleSidebar, subjectData } = this.props;
        ...
        <Formik
          ...
          initialValues={{
            ...
            classData: data?.class && data?.className
              ...
              : "",
            subject: subjectData ? subjectData.map((sub)=>(
              {
                value: sub.value,
                label: sub.value,
                id: sub.id,
          ...
          validationSchema={formschema_olympiad_exam_tests(data)}
