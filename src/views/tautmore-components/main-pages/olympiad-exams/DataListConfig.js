import React, { Component } from "react";
import {TabContent, TabPane, Nav, NavItem, NavLink} from "reactstrap";
import classnames  from "classnames";
import ExamsListing from "./ExamsListing";
import '../exam-list/data-list/ExamList.scss';
import "../../../../assets/scss/pages/data-list.scss";
import '../exam-list/data-list/DataList.scss';
import { history } from "../../../../history";

class DataListConfig extends Component {
  state = {
    active: "1",
    examType: "weekly"
  };
  

  toggle = (tab,param) => {
    console.log(param)
    if (this.state.active !== tab) {
      this.setState({ active: tab });
      this.setState({ examType:param })
      history.push("/olympiad-exams");
    }
  };
  render() {
    return (
      
      <div className={`data-list ${"list-view"}  rp-manage-schools-main`}>
      {/* { this.props.renderFilters() } */}
      <Nav tabs className="nav-justified examsAndTestTab">
      {/* <div className="exam-nav-bottom-border"></div> */}
       
       
        <NavItem>
          <NavLink
            className={classnames({
              active: this.state.active === "1",
            })}
            onClick={() => {
              this.toggle("1", "weekly");
            }}
          >
            Weekly Test
          </NavLink>
        </NavItem>
        <NavItem>
          <NavLink
            className={classnames({
              active: this.state.active === "2",
            })}
            onClick={() => {
              this.toggle("2", "grand-test");
            }}
          >
            Grand Test
          </NavLink>
        </NavItem>
        <NavItem>
          <NavLink
            className={classnames({
              active: this.state.active === "3",
            })}
            onClick={() => {
              this.toggle("3", "chapter-test");
            }}
          >
            Chapter Test
          </NavLink>
        </NavItem>
        <NavItem>
          <NavLink
            className={classnames({
              active: this.state.active === "4",
            })}
            onClick={() => {
              this.toggle("4", "mock-test");
            }}
          >
            Mock Test
          </NavLink>
        </NavItem>
      </Nav>
      
      <TabContent activeTab={this.state.active}>
      <TabPane tabId={this.state.active}>
        <ExamsListing examType={this.state.examType} parsedFilter={this.props.parsedFilter} ></ExamsListing>
      </TabPane>
      </TabContent>
      
      </div>
    );
  }
}

export default DataListConfig;
