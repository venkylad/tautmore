...
const data = JSON.parse(localStorage.getItem('selectedQuedata'));
  const handleQuestionSelect = (event, id, score) => {
    ...
    if (event.target.checked) {
      ...
      localStorage.setItem('selectedQuedata', JSON.stringify(data));
      loadLocalData();
    } else {
  ...
  const redirectToPrevious = (data) =>{
    ...
    history.push(`/add-questions-to-exam-details/${data._id}`)
  }
