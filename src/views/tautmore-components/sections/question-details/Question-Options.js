...
function QuestionOptions(props) {
  ...
  // console.log(props?.qtype, 'bbbbbbbbbbbbbbbbbb ', orientation)
  useEffect(() => {
    ...
    // }
    setOrientation(props?.questionOrientaion)
  ...
  const renderOptions = !checkDragDropSnunscramble(props.qtype) ? (
    ...
    MainData?.options.map((option, key) => {
      return option?.text !== "" || option?.image !== "" ? (
        <ListGroup className={`${orientation}-main`} key={key}>
          ...
          >
            <div className="questionHtlm">
              ...
              {/* {option?.text !== "" && ( */}
              {option?.imageElement !== "" && (
                <div className={option?.image === "" ? "questionHtlm-right" : "questionHtlm-rightImg"}>
                  {/* <span>{option?.text}</span> */}
                  {/* <span className="option-text">{renderText(option?.text)}</span> */}
                  <span className="option-text">{renderText(option?.imageElement)}</span>
                </div>
              )}
            </div>
      ...
      <Row>
        <Col md="12">
          ...
          <h2 className="question-details-queHead">
            <div className="question-desc d-flex">
              {/* 1.  */}
              {props.passageQuestionIndex && props.passageQuestionIndex + ". "}   <span className={props?.qtype === `passage` ? "new_class" : 'no_class'}> {renderText(mainData?.description)}</span>
          ...
          <div className="profile-details">
            <Row>
              {mainData?.solutionType === "fill-in-the-blank" ? (
                ...
                // <Col md="12" className='asdfg' style={{ display: "contents" }}>{renderOptions}</Col>
                <Col md="12" style={{ display: "contents" }}>{renderOptions}</Col>
              )}
