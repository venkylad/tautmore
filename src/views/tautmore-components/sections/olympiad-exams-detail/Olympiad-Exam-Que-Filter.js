...
const ExamQuestionFilter = ({
  ...
  const [subjectOlym, setSubjectOlym] = useState([]);
  ...
  const changeSubject = (data) =>{
      let subjectData = [];
      if(data.id){
        subjectData = [...subjectData,data.id];
      }
      handleSubject(subjectData);
      let urlPrefix = `/add-questions-to-olympiad/${examId}`;
    history.push(`${urlPrefix}?page=1`, subNameId);
  }
  ...
  const getSubjectsOption = (subNameId) =>{
    let subjectsList=[{value: "Subject",
    label: "Subject",
    id: ""}]
    for(const [key,value] of subNameId.id.entries())
    {
        subjectsList=[...subjectsList,{value:subNameId.name[key],label:subNameId.name[key],id:value}]
    }
    setSubjectOlym(subjectsList);
  }

  useEffect(()=>{
      getSubjectsOption(subNameId)
  },[])
    ...
    <div>
      ...
      <div className="filter-section custom-filtersection">
        ...
        <div className="examMarksData">
          ...
          <h3>{localStorage.getItem('Olympiadtotalmarks')}</h3>
          </div>
        ...
        <div className="actions-right rp-manageSchool-head-main d-flex flex-wrap mt-sm-0 mt-2">
          ...
          <UncontrolledDropdown className="data-list-rows-dropdown rp-manageSchool-head mr-1 d-md-block d-none">
            ...
            <DropdownMenu className="customadmindropdown" tag="div" right>
              ...
              <DropdownItem onClick={() => handledifficultyFilter("1")}>
                1
              </DropdownItem>
              <DropdownItem onClick={() => handledifficultyFilter("2")}>
                2
              </DropdownItem>
