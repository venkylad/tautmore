...
class SideMenuContent extends React.Component {
  ...
  render() {
    ...
    const menuItems = navigationConfig.map((item) => {
      // console.log(item.permissions.indexOf(this.props.currentUser) !== -1 ? 'yes' : 'no', 'item permission');
      ...
      let renderItem = (
        ...
        >
          ...
          >
            {item?.title === "Boards" ? (
              <BoardsItem open={this.state.openDropdown} />
            ) : (
              <div className="menu-text">
                {item.icon}
                <span className="tautmore-custom-sidebar-header">
                  {item.title}
                </span>
              </div>
            )}
